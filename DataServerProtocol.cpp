...
void GDCustomJewelBankRecv(SDHP_CUSTOM_JEWELBANK_RECV* lpMsg) // OK
{
	...
	{
		gQueryManager.Close();
		gQueryManager.ExecQuery("INSERT INTO CustomJewelBank (AccountID,Bless,Soul,Life,Creation,Chaos) VALUES ('%s',%d,%d,%d,%d,%d)",lpMsg->account,Bless,Soul,Life,Creation,Chaos);
		gQueryManager.Close();
	...
	{
		gQueryManager.Close();
		gQueryManager.ExecQuery("Update CustomJewelBank SET Bless+=%d,Soul+=%d,Life+=%d,Creation+=%d,Chaos+=%d WHERE AccountID = '%s'",Bless,Soul,Life,Creation,Chaos,lpMsg->account);
		gQueryManager.Close();
	}
...
void DataServerProtocolCore(int index,BYTE head,BYTE* lpMsg,int size) // OK
{
	...
	switch(head)
	{
		...
		{
		case 0x00:
			gCSubasta.DGRequestDataSend((GD_SUBASTAR_REQUEST_DATA_RECV*)lpMsg, index);
			break;
		case 0x01:
			gCSubasta.DGRequestBuyItem((GD_SUBASTA_BUY_ITEM_RECV*)lpMsg, index);
			break;
		case 0x02:
			gCSubasta.DGDeleteItemBuy((DG_SUBASTA_DELETE_ITEM_RECV*)lpMsg, index);
			break;
		case 0x03:
			gCSubasta.DGSendPostItem((GD_SUBASTAR_POST_ITEM_RECV*)lpMsg, index);
			break;
		case 0x04:
			gCSubasta.DGAddReward((GD_SUBASTAR_ADD_REWARD_RECV*)lpMsg, index);
			break;
		case 0x05:
			gCSubasta.DGRequestReward((GD_SUBASTAR_REWARD_RECV*)lpMsg, index);
			break;
		case 0x06:
			gCSubasta.DGDeleteReward((GD_SUBASTAR_DELETE_REWARD_RECV*)lpMsg, index);
			break;
		}
...
void GDCharacterInfoRecv(SDHP_CHARACTER_INFO_RECV* lpMsg,int index) // OK
{
	...
	else
	{
		...
		pMsg.CheckSkin = gQueryManager.GetAsInteger("CheckSkin");
#endif
...
void GDCharacterInfoSaveRecv(SDHP_CHARACTER_INFO_SAVE_RECV* lpMsg) // OK
{
	...
	#if SKIN
	gQueryManager.ExecQuery("UPDATE Character SET Kills=%d, Deads=%d, RentSkin=%d,CheckSkin=%d WHERE Name='%s'", lpMsg->Kills, lpMsg->Deads, lpMsg->RentSkin,lpMsg->CheckSkin, lpMsg->name);
	gQueryManager.Close();
#else
